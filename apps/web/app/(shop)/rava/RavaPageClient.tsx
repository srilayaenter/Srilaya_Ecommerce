...
export default function RavaPageClient({ productImageMap = {} }: RavaPageClientProps) {
  ...
  function addToOrder(id: string) {
    setQuantities((prev) => ({ ...prev, [id]: prev[id] || "1kg" }));
    document.getElementById("order-builder")?.scrollIntoView({ behavior: "smooth" });
  }

  function sendWhatsAppOrder() {
    const selected = PRODUCTS.filter((p) => quantities[p.id] && quantities[p.id] !== "0");
    if (selected.length === 0) {
      alert("Please select at least one quantity before sending.");
      return;
    }
    let msg = "Hello SriLaYa Naturals,\n\nI would like to order the following Millet Rava:\n";
    selected.forEach((p) => {
      msg += `- ${p.name}: ${quantities[p.id]}\n`;
    });
  ...
  return (
    <div className="bg-[#FDFBF7] text-[#2D2722] min-h-screen">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-amber-100/70 via-amber-50/40 to-[#FDFBF7] py-16 md:py-24 border-b border-amber-200/50">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            ...
            <a
              href="#order-builder"
              className="bg-[#D99B26] hover:bg-amber-600 text-[#5C3A21] font-bold px-8 py-3.5 rounded-xl text-base shadow-md transition-all duration-300 flex items-center gap-2"
            >
              🧮 Quick Order
            </a>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-20">
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-[#2E6F40]/10 hover:bg-[#2E6F40] hover:text-white text-[#2E6F40] font-bold rounded-lg text-sm transition-colors flex items-center justify-center gap-2 border border-[#2E6F40]/30 mt-2"
                  >
                    + Add to Quick Order
                  </button>
                </div>
        ...
        {/* Quick Order Builder */}
        <section id="order-builder" className="bg-white rounded-3xl p-8 md:p-12 border border-amber-200 shadow-xl scroll-mt-24">
          <div className="max-w-3xl mx-auto text-center mb-8">
            <span className="text-[#5C3A21] font-bold text-xs uppercase tracking-widest bg-amber-100 px-3 py-1 rounded-full">
              Easy Ordering
            </span>
            <h2 className="font-serif text-3xl font-bold text-[#5C3A21] mt-2">
              Build Your Order via WhatsApp
            </h2>
            <p className="text-gray-500 text-sm mt-1">
              Select quantity for each variety and tap &quot;Send via WhatsApp&quot;. We&apos;ll confirm availability and pricing.
            </p>
          </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {PRODUCTS.map((p) => (
                <div
                  key={p.id}
                  className={`flex items-center justify-between p-3 bg-amber-50/50 rounded-xl border border-amber-200 ${
                    p.wide ? "sm:col-span-2" : ""
                  }`}
                >
                  <span className="font-semibold text-sm text-[#5C3A21]">{p.name}</span>
                  <select
                    value={quantities[p.id] || "0"}
                    onChange={(e) => setQuantities((prev) => ({ ...prev, [p.id]: e.target.value }))}
                    className="border border-gray-300 rounded-lg text-sm px-2 py-1 bg-white focus:outline-none focus:ring-2 focus:ring-[#D99B26]"
                  >
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              <button
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-3 text-base"
              >
