      ...
      <div className="max-w-lg mx-auto px-4 py-12">
        {done ? (
          <div className="bg-white rounded-2xl border border-[#E0E0E0] p-8 text-center">
            <HandsPraying size={40} weight="regular" className="text-naturals-green mx-auto mb-3" />
            ...
            <div className="flex gap-3 justify-center">
              ...
              <Link href="/" className="bg-naturals-green text-white font-bold px-5 py-2 rounded-xl text-sm hover:bg-naturals-green-dark transition-colors">
                Back to shop
              </Link>
            </div>
          </div>
            ...
            <form onSubmit={submit} className="space-y-5">
                  ...
                  {TYPES.map(t => (
                    <button type="button" key={t.value} onClick={() => setType(t.value)}
                      className={`text-left px-4 py-3 rounded-xl border transition-colors ${
                        type === t.value
                          ? "border-naturals-green bg-[#F0FAF5]"
                          : "border-[#E0E0E0] hover:border-[#BDBDBD]"
                      }`}>
                      <p className={`text-sm font-semibold ${type === t.value ? "text-naturals-green" : "text-[#212121]"}`}>{t.label}</p>
              ...
              <div>
                ...
                <textarea value={message} onChange={e => setMessage(e.target.value)} required rows={5}
                  ...
                  className="w-full border border-[#E0E0E0] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-naturals-green resize-none" />
              </div>
              ...
              <button type="submit" disabled={loading || message.trim().length < 10}
                className="w-full bg-naturals-green text-white font-bold py-2.5 rounded-xl text-sm hover:bg-naturals-green-dark transition-colors disabled:opacity-50">
