      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className="bg-white rounded-2xl p-6 shadow-sm border border-[#E0E0E0] flex flex-col hover:shadow-md transition-shadow duration-300"
            >
              {/* Stars */}
              <div className="flex gap-0.5 mb-4">
                {Array.from({ length: 5 }).map((_, idx) => (
                  <span
                    key={idx}
                    className={`text-base ${idx < t.rating ? "text-amber-400" : "text-[#E0E0E0]"}`}
                  >
                    ★
                  </span>
                ))}
              ...
              <p className="text-[#616161] text-sm leading-relaxed flex-grow -mt-2">
                {t.quote}
              </p>
              ...
              <div className="flex items-center gap-3 mt-6 pt-5 border-t border-[#F0F0F0]">
                ...
                <div>
                  ...
                  <p className="text-[#9E9E9E] text-xs font-medium">{t.location}</p>
                </div>
              </div>
            </div>
          ))}
